...
async def fetch_article(url: str, output_dir: Path) -> None:
    print(f"🔄 正在抓取: {url}")
...
def main():
    parser = argparse.ArgumentParser(description="微信公众号文章抓取并转换为 Markdown")
    parser.add_argument("url", help="微信文章 URL (https://mp.weixin.qq.com/...)")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
    ...
    try:
        asyncio.run(fetch_article(args.url, args.output))
    except Exception as e:
        print(f"❌ 抓取失败: {e}")
        sys.exit(1)
