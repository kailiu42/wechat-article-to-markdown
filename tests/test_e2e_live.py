...
def test_live_articles_end_to_end(tmp_path: Path) -> None:
    ...
    for url in urls:
        asyncio.run(asyncio.wait_for(wtm.fetch_article(url, out_dir), timeout=timeout))

    md_files = list(out_dir.rglob("*.md"))
    assert md_files, "Expected at least one markdown file from e2e fetch"

    for url in urls:
        assert _contains_url(md_files, url), (
            f"Expected markdown to include source URL: {url}"
        )
